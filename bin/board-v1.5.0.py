...
def cmd_add(name: str):
    ...
    print()
    print("  ML Platform (optional — press Enter to skip)")
    ml_platform   = input("  ML platform [edge-impulse]: ").strip() or "edge-impulse"
    ml_api_key    = input("  ML API key (Enter to skip): ").strip()
    ml_project_id = input("  ML project ID (Enter to skip): ").strip()
    ...
    print("  Repo:      " + repo)
    if ml_api_key:
        print("  ML platform:    " + ml_platform)
        print("  ML project ID:  " + (ml_project_id if ml_project_id else "(not set)"))
        print("  ML API key:     (stored in config)")
    else:
        print("  ML platform:    (not configured)")
    if is_first:
    ...
    boards[name] = {
        ...
        "apps_path":     apps_path,
        "repo":          repo,
        "ml_platform":   ml_platform   if ml_api_key else "",
        "ml_api_key":    ml_api_key,
        "ml_project_id": ml_project_id if ml_api_key else "",
    }
...
def cmd_show():
    ...
    print("  apps_path: " + info.get("apps_path", "(not set)"))
    print("  repo:      " + info.get("repo", "(not set)"))
    ml_api_key = info.get("ml_api_key", "")
    if ml_api_key:
        print("  ml platform:    " + info.get("ml_platform", "(not set)"))
        print("  ml project ID:  " + info.get("ml_project_id", "(not set)"))
        print("  ml api key:     (configured)")
    else:
        print("  ml platform:    (not configured)")
