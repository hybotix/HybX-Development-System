...
"""
update-v2.1.0.py
Hybrid RobotiX — HybX Development System Updater v2.1

Updates an existing HybX Development System installation.
Pulls the latest repos, refreshes versioned symlinks in ~/bin,
and ensures the HybX venv exists with required dependencies.

v2.1: Docker removed. hybx_app added to shared library.
      HybX venv (~/.hybx/venv) managed by update.
      hybx_runner removed.
...
# HybX venv — owns all Python dependencies for running apps.
# Apps run with this interpreter. update ensures it exists and is current.
VENV_PATH = os.path.expanduser("~/.hybx/venv")
VENV_PYTHON = os.path.join(VENV_PATH, "bin", "python3")
VENV_PIP    = os.path.join(VENV_PATH, "bin", "pip")

# Python packages required in the HybX venv.
VENV_PACKAGES = [
    "msgpack",
]
...
def ensure_venv():
    """
    Ensure the HybX venv exists at VENV_PATH with all required packages.
    Creates the venv if missing. Installs any missing packages.
    ...
    if not os.path.exists(VENV_PYTHON):
        print("  Creating HybX venv...")
        _venv.create(VENV_PATH, with_pip=True, clear=False)
        print("  Created: " + VENV_PATH)

    # Install any missing packages
    for pkg in VENV_PACKAGES:
        code, out = run_quiet([VENV_PYTHON, "-c", f"import {pkg}"])
        if code != 0:
            print("  Installing: " + pkg)
            run_quiet([VENV_PIP, "install", pkg, "-q"])
...
def main():
    ...
    # Sync tracked app files from UNO-Q repo to ~/Arduino/
    # Always updates .ino, .yaml, .py, .md — never touches .cache/
    ...
    if active:
        ...
        if repo_url:
            ...
            arduino_src = os.path.join(repo_dest, repo_name, "Arduino")
    ...
    # Ensure HybX venv exists with required packages
    ensure_venv()
