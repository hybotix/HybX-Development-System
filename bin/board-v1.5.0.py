...
Usage:
  ...
  board ei on             - Enable Edge Impulse integration (prompts for API key)
  board ei off            - Disable Edge Impulse integration
  board ei show           - Show Edge Impulse integration status
...
def cmd_ei(subcommand: str):
    config = load_config()
    ei     = config.setdefault("edge_impulse", {})

    if subcommand == "on":
        print("Enabling Edge Impulse integration.")
        print()
        api_key = input("  Edge Impulse API key: ").strip()
        if not api_key:
        ...
        save_config(config)
        print()
        print("Edge Impulse integration enabled.")
        print("New projects will include Edge Impulse inference scaffolding.")

    elif subcommand == "off":
        ei["enabled"] = False
        save_config(config)
        print("Edge Impulse integration disabled.")
        print("API key retained in config — run 'board ei on' to re-enable.")

    elif subcommand == "show":
        enabled = ei.get("enabled", False)
        api_key = ei.get("api_key", "")
        masked  = (api_key[:8] + "..." + api_key[-4:]) if len(api_key) > 12 else "***"
        print("Edge Impulse integration: " + ("ENABLED" if enabled else "DISABLED"))
        if api_key:
        ...
        if not enabled:
            print()
            print("Run 'board ei on' to enable.")

    else:
        print("Usage:")
        print("  board ei on    - Enable Edge Impulse integration")
        print("  board ei off   - Disable Edge Impulse integration")
        print("  board ei show  - Show Edge Impulse integration status")
        sys.exit(1)
