...
New in v1.1.0:
  - Full pathing tests for all commands that accept a path or name:
      build: no args, project name, full path
      start/stop/restart/logs/clean: app name, no args (last app fallback)
  - ~/lib/ deployment verification
  - ~/bin/ cleanliness verification — no old versions, no retired commands
...
Usage:
  python3 test-v1.1.0.py           — Run all tests (read-only + hardware)
  python3 test-v1.1.0.py --all     — Include sandboxed tests
  python3 test-v1.1.0.py --verbose — Show full command output
...
def check(name: str, condition: bool, fail_msg: str = ""):
    """Record a pass/fail for a non-command check."""
    if condition:
        passed.append(name)
        print("  PASS  " + name)
    else:
...
# ── v1.1 Infrastructure Tests ──────────────────────────────────────────────────


def test_v11_infrastructure():
    header("v1.1 infrastructure")

    # ~/lib/ exists
    check("~/lib/ exists",
          os.path.isdir(LIB_DIR),
          "~/lib/ not found — run update")

    # Shared modules deployed to ~/lib/
    for module in ["hybx_config.py", "libs_helpers.py"]:
        check("~/lib/" + module + " deployed",
              os.path.exists(os.path.join(LIB_DIR, module)),
              module + " not found in ~/lib/")

    # Shared modules NOT in ~/bin/ (old architecture)
    for module in ["hybx_config.py", "libs_helpers.py", "ml_helpers.py"]:
        check("~/bin/" + module + " removed",
              not os.path.exists(os.path.join(BIN_DIR, module)),
              module + " still in ~/bin/ — should have been cleaned up by update")

    # Retired commands NOT in ~/bin/
    for retired in ["cache", "boardsync"]:
        check("retired '" + retired + "' not in ~/bin/",
              not os.path.exists(os.path.join(BIN_DIR, retired)),
              retired + " still in ~/bin/ — should have been purged by update")
        # Check no versioned files either
        old_files = [f for f in os.listdir(BIN_DIR)
                     if f.startswith(retired + "-v") and f.endswith(".py")]
        check("no retired '" + retired + "' versioned files in ~/bin/",
              len(old_files) == 0,
              "found: " + ", ".join(old_files))

    # Only ONE versioned file per active command in ~/bin/
    all_commands = ["board", "build", "clean", "libs", "list", "logs",
                    "migrate", "project", "restart", "setup", "start",
                    "stop", "update"]
    for cmd in all_commands:
        versioned = [f for f in os.listdir(BIN_DIR)
                     if f.startswith(cmd + "-v") and f.endswith(".py")]
        check("~/bin/ has exactly one version of " + cmd,
              len(versioned) == 1,
              "found " + str(len(versioned)) + " versions: " + ", ".join(versioned))
...
def test_build():
    ...
    if not os.path.isdir(app_path):
        skip("build — all pathing modes", TEST_APP + " not found at " + app_path)
        return

    # ── Three ways to call build ───────────────────────────────────────────────

    # 1. Full sketch path
    test("build — full sketch path",
         ["build", sketch_path],
         expect_in=["build"],
         timeout=300)

    # 2. Project name only — the bug we fixed in v1.1
    test("build — project name only",
         ["build", TEST_APP],
         expect_in=["build"],
         timeout=300)

    # 3. No args — uses active project
    # First set scd30 as active project
    run_cmd(["project", "use", TEST_APP])
    test("build — no args (uses active project)",
         ["build"],
         expect_in=["build"],
         timeout=300)
...
def test_app_lifecycle():
    header("start / stop / restart / logs / clean — all pathing modes")
    ...
    test("start — no args (uses last app)",
         ["start"],
         expect_in=["start"],
         timeout=120)
    time.sleep(5)
    ...
    test("logs — no args (uses last app)",
         ["logs"],
         timeout=10)

    # ── stop with no args — uses last app ─────────────────────────────────────
    test("stop — no args (uses last app)",
         ["stop"],
         expect_in=["stop"],
         timeout=30)
    time.sleep(2)

    # ── restart with app name ──────────────────────────────────────────────────
    test("restart — app name",
         ["restart", TEST_APP],
         expect_in=["start"],
         timeout=120)
    time.sleep(5)

    # ── restart with no args — uses last app ──────────────────────────────────
    test("restart — no args (uses last app)",
         ["restart"],
         expect_in=["start"],
         timeout=120)
    time.sleep(5)
    ...
    # ── clean with no args — uses last app ────────────────────────────────────
    run_cmd(["start", TEST_APP])
    time.sleep(10)
    run_cmd(["stop", TEST_APP])
    time.sleep(2)
    test("clean — no args (uses last app)",
         ["clean"],
         expect_in=["clean"],
         timeout=120)
...
def test_sandboxed_board():
    ...
    check("board add cancelled — config unchanged",
          test_board_name not in config_after.get("boards", {}),
          "board was added to config despite cancellation!")
...
def test_sandboxed_project():
    ...
    # Verify directory and scaffold
    check("project new — directory created",
          os.path.isdir(test_project_path),
          "directory not found at " + test_project_path)

    for f in ["app.yaml", "sketch/sketch.ino", "sketch/sketch.yaml", "python/main.py"]:
        check("project new — scaffold: " + f,
              os.path.exists(os.path.join(test_project_path, f)),
              f + " not found in scaffold")
    ...
    check("project remove cancelled — directory still exists",
          os.path.isdir(test_project_path),
          "directory was removed despite cancellation!")
    ...
    if not os.path.isdir(test_project_path):
        check("project remove — directory removed", True)
    else:
        check("project remove — directory removed", False, "directory still exists")
        shutil.rmtree(test_project_path)
