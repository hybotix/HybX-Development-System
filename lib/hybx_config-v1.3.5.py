...
def pull_repo(dest: str):
    """
    Pull the latest changes for a repo on its currently checked-out branch.

    Never switches branches. Pulls whatever branch is active in the repo.
...
def pull_all_repos(config: dict):
    ...
    # Pull Dev System on whatever branch is currently checked out.
    # Never switch branches — that is the developer's responsibility.
    pull_repo(dev_dest)
...
# Minimum number of characters required for prefix command matching.
...
# ── Versioned file symlinks ────────────────────────────────────────────────────

def _ver_tuple(fname: str) -> tuple:
    """Extract version tuple from a versioned filename for sorting."""
    import re
    m = re.search(r"-v(\d+)\.(\d+)\.(\d+)\.py$", fname)
    return tuple(int(x) for x in m.groups()) if m else (0, 0, 0)


def symlink_versioned_files(project_path: str):
    """
    Scan a project directory (and its immediate subdirectories) for versioned
    Python files matching the pattern name-vX.Y.Z.py and create a bare-name
    symlink pointing to the latest version.

    This is the app-level equivalent of what update does for ~/bin and ~/lib.
    Called by project pull after copying files from the repo to the board.

    Example:
        python/main-v1.0.1.py   →  python/main.py -> python/main-v1.0.1.py
        visualizer-v1.0.1.py    →  visualizer.py  -> visualizer-v1.0.1.py

    Symlinks are never stored in the repo — they are created on the board
    by project pull. project push skips symlinks when copying to the repo.
    """
    import re

    # Scan project root and all immediate subdirectories (except .cache)
    ...
    for scan_dir in scan_dirs:
        ...
        # Collect versioned files grouped by bare name
        versioned = {}
        ...
        for bare, files in versioned.items():
            files.sort(key=_ver_tuple)
            latest      = files[-1]
            latest_path = os.path.join(scan_dir, latest)
            bare_path   = os.path.join(scan_dir, bare + ".py")

            # Update symlink if missing or pointing to wrong version
            current = os.readlink(bare_path) if os.path.islink(bare_path) else None
            ...
            # Make executable — only files in the app root, not subdirectories
            if scan_dir == project_path:
                try:
...
def install_sigint_handler(message: str = "\nInterrupted."):
    """
    Install a clean SIGINT (Ctrl+C) handler for all HybX commands.

    Prints a short message and exits with code 1.
    Call once at the top of every command's main().

    Usage:
        from hybx_config import ..., install_sigint_handler
