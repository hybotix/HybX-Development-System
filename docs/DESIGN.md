...
|---------|--------|-------------|
| `start` | v0.0.6 | Nuke Docker, clear cache, patch `$HOME` mount, install newrepo, launch app |
...
Sequence:
1. Run `arduino-cli compile --fqbn arduino:zephyr:unoq <sketch_path> -v`
2. Parse "Used library" lines from verbose output to extract library names and versions
3. Auto-generate `sketch.yaml` with the detected libraries
...
Wraps `arduino-cli lib` commands:
