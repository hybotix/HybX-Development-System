...
class HybXCompiler:
    ...
    def _discover_libraries(self, cpp_path: str,
        ...
        paths. Searches: project libs (<app>/libraries/), user libs (~/.Arduino/libraries), platform libs,
    ...
    def _resolve_header(self, header: str) -> Optional[str]:
        ...
        search_paths = [
            os.path.join(self.app_path, "libraries"),  # project-specific — first
            self._user_libs,
            self._platform_libs,
        ]
    ...
    def _find_sources(self, lib_path: str) -> list[str]:
        ...
        for pattern in ["*.cpp", "*.c"]:
            ...
            if os.path.isdir(os.path.join(lib_path, "src")):
                ...
                if os.path.isdir(os.path.join(lib_path, "src", "uld")):
                    sources.extend(glob.glob(os.path.join(lib_path, "src", "uld", pattern)))
