...
def pull_all_repos(config: dict):
    ...
    # Never hardcode a branch — respect wherever the repo is pointed.
    code, current_branch = _run_quiet(
        ["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=dev_dest)
    dev_branch = current_branch.strip() if code == 0 and current_branch.strip() else "main"
    pull_repo(dev_dest, branch=dev_branch)
