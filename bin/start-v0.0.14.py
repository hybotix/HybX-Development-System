...
def stop_app(app_path: str, app_id: str):
    """
    Stop the running app via arduino-app-cli and wait until the
    Docker container is fully gone before returning.
    Delegates to the stop command so container polling is consistent.
    """
    container_name = "arduino-" + app_id + "-main-1"
    print("Stopping app: " + app_id)
    subprocess.run(["arduino-app-cli", "app", "stop", app_path])

    # Poll until container is gone
    ...
    while True:
        ...
        time.sleep(1)
    print()
    # Poll arduino-app-cli status until it no longer reports the app
    # as running. arduino-app-cli tracks app state independently of
    # Docker and must fully release before start can proceed.
    print("Waiting for app state to settle...", end="", flush=True)
    for _ in range(60):
        time.sleep(1)
        print(".", end="", flush=True)
        result = subprocess.run(
            ["arduino-app-cli", "app", "status", app_path],
            capture_output=True,
            text=True,
        )
